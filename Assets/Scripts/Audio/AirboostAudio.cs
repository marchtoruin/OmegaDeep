...
public class AirboostAudio : MonoBehaviour
{
    ...
    private PlayerStateMachine stateMachine; // Reference to state machine
    private EventInstance currentBoostStartInstance; // Track the currently playing boost start sound

    private void Awake()
    {
        // Get state machine reference
        stateMachine = GetComponentInParent<PlayerStateMachine>(); // Assuming it's on the player parent
        if (stateMachine == null)
        {
            Debug.LogWarning("AirboostAudio: PlayerStateMachine not found! Boost enable/disable audio might not work.", this);
        }
    ...
    private void Update()
    {
        ...
        // --- Check if Player is in Death State --- 
        if (stateMachine != null && stateMachine.CurrentState is PlayerDeathState)
        {
            // If player died mid-boost, stop the start sound immediately
            if (isBoostActive && currentBoostStartInstance.isValid())
            {
                currentBoostStartInstance.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
                currentBoostStartInstance = default; // Clear the tracked instance
                Debug.Log("[AirboostAudio] Player died mid-boost. Stopped boost start sound.");
            }
            isBoostActive = false; // Ensure boost is marked inactive
            return; // Stop processing audio input
        }
        // ----------------------------------------
        ...
        // Check if boost is allowed by the state machine
        bool boostAllowed = (stateMachine == null || stateMachine.IsBoostAllowed);

        // --- Handle case where boost becomes disallowed while active ---
        if (!boostAllowed && isBoostActive)
        {
            PlayBoostStopSound();
            isBoostActive = false;
            // Skip further input checks for this frame
            return; 
        }

        // --- Process Input only if boost is allowed ---
        if (boostAllowed)
        {
            // Detect boost key press
            if (Input.GetKeyDown(boostKey))
            {
                // Only play start sound if not already boosting
                if (!isBoostActive)
                {
                    PlayBoostStartSound();
                    isBoostActive = true;
        ...
        // If boost is NOT allowed, isBoostActive should remain false or be forced false above
    }
    ...
    private void PlayBoostStartSound()
    {
        ...
        for (int i = 0; i < boostStartPool.Count; i++)
        {
            ...
            if (state != FMOD.Studio.PLAYBACK_STATE.PLAYING)
            {
                ...
                currentBoostStartInstance = boostStartPool[i]; // Track this instance
        ...
        if (!foundInstance)
        {
            ...
            // PlayOneShot is harder to stop, so we still create an instance but don't pool it long term
            currentBoostStartInstance = RuntimeManager.CreateInstance(boostStartEvent);
            currentBoostStartInstance.set3DAttributes(RuntimeUtils.To3DAttributes(gameObject));
            currentBoostStartInstance.start();
            currentBoostStartInstance.release(); // Release immediately after starting
        }
    }

    private void PlayBoostStopSound()
    {
        // Clear the tracked boost start instance since boost is stopping
        if(currentBoostStartInstance.isValid())
        {
            // Optionally, ensure the start sound is actually stopped if logic requires immediate cutoff
            // currentBoostStartInstance.stop(FMOD.Studio.STOP_MODE.ALLOWFADEOUT); 
            currentBoostStartInstance = default;
        }
